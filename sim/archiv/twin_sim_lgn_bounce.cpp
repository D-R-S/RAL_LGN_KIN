...
// ── 2D projection of every link's world origin, for gnuplot. We project
//    onto the X-Y plane (gravity points along -Y) so the swing motion
...
static StickFigure poses_lgn(lgn::KinematicTree& tree,
    ...
    tree.fk(q);
    StickFigure f;
    for (int i = 0; i < tree.n_links(); ++i) {
        lgn::Vec3 p = lgn::p_of(tree.link(i).T_world);
        f.xs.push_back(p.x());
        f.ys.push_back(p.y());
    }
    return f;
...
static StickFigure poses_pin(pinocchio::Model& model, pinocchio::Data& data,
    ...
    StickFigure f;
    // Pinocchio's oMi is per-joint; we walk model.njoints to get every frame.
    for (pinocchio::JointIndex j = 0; j < (pinocchio::JointIndex)model.njoints; ++j) {
