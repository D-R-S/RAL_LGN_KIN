...
static StickFigure poses_pin(pinocchio::Model& model, pinocchio::Data& data,
                             const Eigen::VectorXd& q) {
    // updateFramePlacements requires forwardKinematics to have been called.
    ...
    // Tip: read the URDF's "tip" link directly out of pinocchio's frame
    // table. pinocchio preserves URDF link names as body-type frames in
    // model.frames; this is the same geometric point lgn reads from its
    // "tip" link's T_world.
    pinocchio::FrameIndex tip_id = model.getFrameId("tip");
    const auto& t_tip = data.oMf[tip_id].translation();
    f.xs.push_back(t_tip[0]);
    f.ys.push_back(t_tip[1]);
    return f;
}
