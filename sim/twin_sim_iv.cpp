...
int main(int argc, char** argv) {
    ...
    for (int s = 0; s < steps; ++s) {
        ...
        if (plot) {
            ...
            //
            //  Plus a horizontal dashed line at y = ΔE_tot = |E_pin - E_lgn|
            //  where E_engine = sum_i (KE_i + PE_i). This visualizes how
            //  much the two engines disagree on TOTAL system energy in real
            //  time — flat near zero while integrators stay coherent, rises
            //  visibly once chaos kicks in.
            //
            //  PE reference is time-constant (h_ref = n·L) so the curves
            //  don't drift on the y-axis between frames.
            const double h_ref = n * link_L;
            const double pe_shift_const = link_m * 9.81 * h_ref;
            // Total energies and their disagreement
            ...
            std::fprintf(gp1,
                "set yrange [%f:%f]\n"
                "set title sprintf('KE / PE per link   t = %%.2f s   |ΔE_{tot}| = %%.2e J', %f, %g)\n"
                "plot '-' w linespoints lt 1 lw 2.5 pt 7 ps 1.3 lc rgb '#1F5FA8' t 'KE  pin', "
                     ...
                     "'-' w lines lt 2 lw 2 lc rgb '#B2182B' t '|ΔE_{tot}|'\n",
            ...
            std::fprintf(gp1, "e\n");
            // |ΔE_tot| as horizontal line spanning the full x range
            std::fprintf(gp1, "%d %f\n", 0,   dE_tot);
            std::fprintf(gp1, "%d %f\n", n-1, dE_tot);
            std::fprintf(gp1, "e\n");
            std::fflush(gp1);
